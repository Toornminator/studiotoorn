  ...
  return (
    <>
      <ul className="mt-16 space-y-px border-t border-b border-ink/15 md:mt-24">
        {events.map((event) => {
          const d = formatDate(event.date, locale);
          const isSoldOut =
          ...
          return (
            <li key={event.slug}>
              <motion.button
                ...
                data-cursor={isSoldOut ? t.events.cursorSoldOut : t.events.cursorBook}
              ...
              >
                <div>
                  ...
                  >
                    {d.day} <span className="text-ink/65">{d.month}</span>
                  </div>
                ...
                <div className="hidden flex-col items-end font-mono text-[10px] uppercase tracking-[0.22em] md:flex">
                  {isSoldOut ? (
                    <span className="text-ink/40">{t.events.soldOut}</span>
                  ) : event.spotsAvailable !== undefined && event.capacity ? (
                    <>
                      <span className="text-ink">
                        {event.spotsAvailable} / {event.capacity}
                      </span>
                      <span className="text-ink/40">{t.events.spotsSuffix}</span>
                    </>
                  ) : (
                    <span className="text-ink/40">{t.events.onRequest}</span>
                  )}
                </div>
                ...
                <span className="inline-flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.22em] text-ink/45 transition-colors group-hover:text-tattoo-red">
                  <span className="hidden md:inline">{t.events.openLabel}</span>
