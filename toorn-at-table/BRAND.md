...
**Visual one-pager:** [`docs/brand-board.pdf`](docs/brand-board.pdf)
(downloadable, near-A4) · [`docs/brand-board.png`](docs/brand-board.png) ·
rendered from [`docs/brand-board.html`](docs/brand-board.html), which uses the
real fonts and exact hex values, so it can be re-exported any time.
